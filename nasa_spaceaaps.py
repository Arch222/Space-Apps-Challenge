...
# NO2 graphs
df_no2 = pd.read_csv('//uahdata/rstor/NASA-COVID19-hackathon-2020/DATA/NO2/extract_NO2_1.csv')
...
for item in cities:
    ...
    '''
    #print(df_no2_nyc.resample('M').mean())
    df_no2_nyc_resample = df_no2_nyc.resample('M').mean()
    df_no2_nyc_resample['Month'] = df_no2_nyc_resample.index.month
    df_no2_nyc_resample_grp = df_no2_nyc_resample.groupby('Month')
    if item == 'NYC':
        for i in range(2,6):
            print(df_no2_nyc_resample_grp.get_group(i).diff())
    '''
...
for item in keys:
    ...
    #print(item)
    #print(df_item.resample('M').mean())
    
    '''
    #print(df_item.resample('M').mean())
    df_item_res = df_item.resample('M').mean()
    df_item_res['Month'] = df_item_res.index.month
    df_item_res = df_item_res.groupby('Month')
    if item == 'NYC':
        for i in range(2,6):
            print(df_item_res.get_group(i))
    '''        
...
for item in cities:
    ...
    #print(df_chl_item.resample('M').mean())
    
    
    #print(df_item.resample('M').mean())
    df_chl_item_res = df_chl_item.resample('M').mean()
    df_chl_item_res['Month'] = df_chl_item_res.index.month
    df_chl_item_res = df_chl_item_res.groupby('Month')
    if item == 'NYC':
        for i in range(2,6):
            print(df_chl_item_res.get_group(i))
...
plt.plot(x,y_b,'--',c='black')
plt.ylim(-100,10)
plt.ylabel('Diffence of CHL (2019 - 2020) (%)',size=20)
plt.title('Changes in Water Quality at Lower New York Bay (40.51,-74.03)',size=20)
plt.tick_params(axis='x',labelsize=20,rotation=35)
...
cities = ['NYC','Wuhan','Milan']
for item in cities:
    df_item = df[['system:time_start',item]].dropna()
    df_item['system:time_start'] = pd.to_datetime(df_item['system:time_start'],
              format='%d-%b-%y')
    
    df_item = df_item.set_index('system:time_start')
    
    df_item = df_item.rename(columns={'system:time_start':'date_time'})
...
plt.plot(x,y_b,'--',c='black')
plt.ylim(-5,20)
plt.ylabel('Diffence of NDVI (2019 - 2020) (%)',size=20)
plt.title('Changes in NDVI at Lower New York Bay (40.51,-74.03)',size=20)
plt.tick_params(axis='x',labelsize=20,rotation=35)
plt.tick_params(axis='y',labelsize=20)
